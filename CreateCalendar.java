    ...
    public void mainCreateCalendar(List<ReunionDoodle> reunionDoodles){
        try {
            ...
            for(ReunionDoodle reunionDoodle : reunionDoodles) {
                ...
                try {
                    if (reunionDoodle.getReponse().equals(Reponse.ORGANISATEUR)) {
                        VEvent event = createEvent("moi@noodle.com", reunionDoodle.getNom() + " Vous ếtes l'organisateur ", reunionDoodle.getLocalisation(), reunionDoodle.getDate(), reunionDoodle.getHeureDepart(), reunionDoodle.getHeureFin(), timezone);
                        ...
                        VEvent event = createEvent(mailAdresse, reunionDoodle.getNom() + " Vous avez dit : " + reunionDoodle.getReponse(), reunionDoodle.getLocalisation(), reunionDoodle.getDate(), reunionDoodle.getHeureDepart(), reunionDoodle.getHeureFin(), timezone);
                        calendar.getComponents().add(event);
                    }
                }catch (Exception e){
                    System.out.println("Je n'arrive pas à avoir mon choix");
                }
            }
